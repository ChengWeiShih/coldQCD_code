...
MBD_zvtx_effi_scan::MBD_zvtx_effi_scan(
...
void MBD_zvtx_effi_scan::MBD_vtxZ_macro(int total_trials, bool run_simple)
{
...
vector<TGraph *> MBD_zvtx_effi_scan::GetRchi2_TGraph()
{
...
void MBD_zvtx_effi_scan::MakeMBD_vertexZ_hist_func(
...
)
{
    if (apply_effi_corr)
    {
        for (int step : chosen_scan_steps)
        {
            ...
            h1D_detectorNS_vertexZ_vecAll[step] -> Fit(fit_gaus_map_all[step], "NQ","",-250, 250);
...
void MBD_zvtx_effi_scan::DrawTrialPlots(int index, int total_trials)
{
...
void MBD_zvtx_effi_scan::ClearUp()
{
...
void MBD_zvtx_effi_scan::DrawPlots(int selection)
{
    // note : ============================ ============================ ============================ ============================ ============================
...
pair<double, double> MBD_zvtx_effi_scan::FindLowestEntry_gr(T * gr)
{
    double min;
    double min_location;
...
template std::pair<double, double> MBD_zvtx_effi_scan::FindLowestEntry_gr<TGraph>(TGraph *gr);