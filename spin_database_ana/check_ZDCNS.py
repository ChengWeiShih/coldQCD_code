...
def prepare_run_script(run_year_cut, run_type_cut, run_runnumber_cut, ZDC_index):
    run_ZDCcoinevt_script = '''
        SELECT 
            run.runnumber, 
            run.runtype, 
...
def main():
    ...
    parser.add_argument('--text', action='store_true', help='print out the content of the table for the given gl1_scalers index')
    parser.add_argument('--duration', type=float, default=0, help='Duration cut')
    ...
    if args.text:
        ZDC_index = args.N_row  
        run_ZDCcoinevt_script = prepare_run_script(run_year_cut, run_type_cut, run_runnumber_cut, ZDC_index)
    else:
        run_ZDCcoinevt_script = prepare_run_script(run_year_cut, run_type_cut, run_runnumber_cut, ZDC_index)

    run_ZDCevt_df = show_cali_table(daq_cur,daq_conn, run_ZDCcoinevt_script)
    run_ZDCevt_df['scaled_rate'] = (run_ZDCevt_df['scaled'].astype(float) / (run_ZDCevt_df['duration'].astype(float) * 60.0)) # note : Hz
    run_ZDCevt_df['scaled_rate'] = run_ZDCevt_df['scaled_rate'].round(2)
    run_ZDCevt_df['raw_rate'] = (run_ZDCevt_df['raw'].astype(float) / (run_ZDCevt_df['duration'].astype(float) * 60.0)) # note : Hz
    run_ZDCevt_df['raw_rate'] = run_ZDCevt_df['raw_rate'].round(2)
    ...
    if args.category:
        ...
        short_run_ZDCevt_df.loc[(short_run_ZDCevt_df['live_rate'] > good_live_rate) & (short_run_ZDCevt_df['duration'] > good_duration) & (short_run_ZDCevt_df['scaled'] > 10), 'run_tag'] = 1111111
        ...
        if args.duration:
            run_ZDCNSevt_fill_combined_df.query('duration > %i' % args.duration, inplace=True)
        print(run_ZDCNSevt_fill_combined_df.to_string())
    ...
    if args.RatePlot:
        ...
        # plt.scatter(run_ZDCevt_df_cut['brtimestamp'], run_ZDCevt_df_cut['scaled_rate'], label='ZDC scaled trigger rate', color = 'orange', s=15)
        plt.plot(run_ZDCevt_df_cut['brtimestamp'], run_ZDCevt_df_cut['scaled_rate'], 'o-', label='ZDC scaled trigger rate', color = 'orange', markersize=5)
